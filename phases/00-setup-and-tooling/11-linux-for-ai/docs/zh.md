...
1. 通过 SSH 连接到任何 Linux 计算机（或打开 WSL2）并导航到您的主目录。创建一个项目文件夹，使用“touch”在其中创建三个空文件，然后使用“ls -la”列出它们。
2. 使用 apt 安装 `htop`，运行它，并确定哪个进程使用最多内存。
3. 启动 tmux 会话，在其中运行 `sleep 300`，分离，列出会话，然后重新附加。
