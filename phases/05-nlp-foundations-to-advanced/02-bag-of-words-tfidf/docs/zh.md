...
1. **简单。** 在 L2 标准化 TF-IDF 输出上实现 `cosine_similarity(doc_vec_a, doc_vec_b)`。验证相同文档的得分为 1.0，词汇不相交的文档得分为 0.0。
2. **中。** 为“bag_of_words”添加“n-gram”支持。参数“n”生成超过“n”-gram 的计数。测试 `["the", "cat", "sat"]` 上的 `n=2` 会生成 `["the cat", "cat sat"]` 的二元组计数。
3. **困难。** 使用 GloVe 100d 向量构建上面的 TF-IDF 加权嵌入混合体（下载一次，缓存）。在 20 个新闻组数据集上将分类准确性与普通 TF-IDF 和普通平均池嵌入进行比较。报告哪一方获胜。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|-----------------|------------------------|
|弓 |词频向量|一份文档中的词汇单词计数。丢弃订单。 |
| TF |术语频率 |文档中单词的计数，可以选择按文档长度进行标准化。 |
| DF |文档频率 |至少包含一次该单词的文档计数。 |
|以色列国防军 |逆文档频率| `log(N / df)` 已平滑。降低随处出现的单词的权重。 |
|稀疏向量|大部分都是零 |词汇量通常为 10k-100k 个单词；大多数都没有出现在任何给定的文件中。 |
|余弦相似度 |矢量角| L2 归一化向量的点积。 1 是相同的，0 是正交的。 |

## 进一步阅读

- [scikit-learn — 从文本中提取特征](https://scikit-learn.org/stable/modules/feature_extraction.html#text-feature-extraction) — 规范的 API 参考，以及每个旋钮上的注释。
- [Salton, G., & Buckley, C. (1988)。自动文本检索中的术语权重方法](https://www.sciencedirect.com/science/article/pii/0306457388900210) - 这篇论文使 TF-IDF 成为十年来的默认值。
- [“为什么 TF-IDF 仍然击败嵌入” — Ashfaque Thonikkadavan（中）](https://medium.com/@cmtwskb/why-tf-idf-still-beats-embeddings-ad85c123e1b2) — 2026 年旧方法何时获胜以及原因。
