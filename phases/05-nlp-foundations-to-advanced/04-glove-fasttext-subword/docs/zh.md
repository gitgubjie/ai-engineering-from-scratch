...
1. **简单。** 运行 `char_ngrams("playing")` 和 `char_ngrams("played")`。计算两个 n 元组的 Jaccard 重叠。您应该看到大量共享片段（“pla”、“lay”、“play”），这就是 FastText 在形态变体之间迁移良好的原因。
2. **中。** 扩展 `learn_bpe` 以跟踪词汇量增长。将每个语料库字符的标记绘制为合并次数的函数。您首先应该看到快速压缩，每个标记渐近约 2-3 个字符。
3. **困难。** 在莎士比亚全集上训练 1k 合并 BPE。比较常用词和罕见专有名词的标记化。测量之前和之后每个单词的平均标记。写下令你惊讶的事情。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|-----------------|------------------------|
|共现矩阵|词-词频表 | “X[i][j]” = 单词“j”出现在单词“i”周围的窗口中的频率。 |
|子词|一句话|字符 n-gram (FastText) 或学习标记 (BPE/WordPiece/SentencePiece)。 |
| BPE |字节对编码 |迭代合并最频繁的相邻对，直到词汇量达到目标大小。 |
| OOV |词汇量不足 |模特从未见过的词。 Word2Vec/GloVe 失败。 FastText 和 BPE 可以处理它。 |
|字节级 BPE |原始字节上的 BPE | GPT-2的方案。词汇表以 256 字节开头，因此不会出现 OOV。 |

## 进一步阅读

- [彭宁顿、索切尔、曼宁 (2014)。 GloVe: Global Vectors for Word Representation](https://nlp.stanford.edu/pubs/glove.pdf) — GloVe 论文，七页，仍然是损失的最佳推导。
- [博亚诺夫斯基等人。 （2017）。用子词信息丰富词向量](https://arxiv.org/abs/1607.04606) — FastText。
- [Sennrich、Haddow、Birch (2016)。带子词单元的生僻词的神经机器翻译](https://arxiv.org/abs/1508.07909) — 这篇将 BPE 引入现代 NLP 的论文。
- [Hugging Face tokenizer 摘要](https://huggingface.co/docs/transformers/tokenizer_summary) — BPE、WordPiece 和 SentencePiece 在实践中有何不同。
