...
1. **凸性图库。** 使用检查器测试这些函数的凸性：f(x) = x^4、f(x) = sin(x)、f(x,y) = x^2 + y^2、f(x,y) = x*y、f(x) = max(x, 0)。解释为什么每个结果都有意义。

2. **牛顿与梯度下降竞赛。** 从起点 (10, 10) 在 f(x,y) = 50*x^2 + y^2 上运行这两种方法。每个需要多少步才能达到损失 < 1e-10？当条件数（最大与最小 Hessian 特征值之比）增加时，梯度下降会发生什么？

3. **拉格朗日乘子几何。** 在 x + 2y = 4 的条件下最小化 f(x,y) = (x-3)^2 + (y-3)^2。通过检查解处 f 的梯度是否与 g 的梯度平行来验证解。

4. **正则化约束。** 实施 L1 约束优化：最小化 (x-3)^2 + (y-2)^2 服从 |x| + |y| <= 1。表明解有一个坐标为零（菱形约束的稀疏性）。

5. **Hessian 特征值分析。** 计算 Rosenbrock 函数在 (1,1) 和 (-1,1) 处的 Hessian。计算两个点的特征值。特征值告诉您关于最小曲率与远离最小曲率的什么信息？

## 关键术语

|术语 |这意味着什么 |
|------|----------------|
|凸集 |一个集合，其中集合中任意两点之间的线段位于集合内 |
|凸函数|一种函数，其图形上任意两点之间的连线位于图形上方或图形上。等价地，Hessian 处处都是正半定 |
|局部最小值|比所有附近点低的点。对于凸函数，每个局部最小值都是全局最小值 |
|全局最小值 |函数在其整个域上的最低点 |
|海森矩阵|所有二阶偏导数的矩阵。编码曲率信息 |
|正半定|特征值均为非负的矩阵。 “二阶导数 >= 0”的多维模拟 |
|条件编号| Hessian 矩阵的最大与最小特征值之比。高条件数意味着拉长的山谷和缓慢的梯度下降|
|牛顿法|使用逆 Hessian 矩阵确定步长方向和大小的二阶优化器。二次收敛接近最小值 |
|拉格朗日乘数|引入变量以将约束优化问题转换为无约束优化问题 |
| KKT 条件 |具有不等式约束的最优性的必要条件。推广拉格朗日乘子 |
|互补松弛 |在解中，约束要么处于活动状态，要么其乘数为零。永远不会都非零 |
|二元性|每个约束问题都有一个伴随的对偶问题。对于凸问题，两者具有相同的最优值 |
|强对偶性 |原始最优值和对偶最优值相等。满足斯莱特条件的凸问题成立 |
| L-BFGS|近似二阶方法，存储最后 m 个梯度差而不是完整的 Hessian |
|鞍点|梯度为零但在某些方向上最小而在其他方向上最大的点 |
|过度参数化 |使用比训练示例更多的参数。平滑损失景观并减少不良的局部最小值 |

## 进一步阅读

- [Boyd & Vandenberghe: Convex Optimization](https://web.stanford.edu/~boyd/cvxbook/) - 标准教科书，在线免费提供
