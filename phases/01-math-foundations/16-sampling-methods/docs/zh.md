...
2. 使用拒绝采样，使用 Uniform(0, 1) 提议从 Beta(2, 5) 分布生成样本。根据真实的 Beta PDF 绘制已接受的样本。理论接受率是多少？
...
|拒绝抽样| “提议并接受/拒绝” |从一个简单的提案生成，以与目标/提案比率成比例的概率接受。准确但浪费样品 |
|重要性抽样| “重新称重样品” |通过 p(x)/q(x) 对每个样本进行加权，使用 q(x) 中的样本估计 p(x) 下的期望。强化学习中 PPO 的核心 |
|蒙特卡洛 | “平均随机样本” |近似积分作为样本平均值。无论维度如何，误差为 O(1/sqrt(N)) |
...
|老化| 「热身期」|在链达到其固定分布之前丢弃初始 MCMC 样本 |
