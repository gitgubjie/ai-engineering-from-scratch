...
- `outputs/prompt-dit-model-picker.md` — 在给定质量、延迟和许可证限制的情况下，在 SD3、FLUX.1-dev、FLUX.1-schnell、Z-Image、SD4 Turbo 之间进行选择。
- `outputs/skill-rectified-flow-trainer.md` — 使用 AdaLN DiT 和 Euler 采样编写校正流的完整训练循环。

## 练习

1. **（简单）** 在合成 blob 数据集上训练上面的 TinyDiT 500 个步骤。比较使用 10、20 和 50 欧拉步骤生成的样本。
2. **（中）** 通过将学习的类嵌入连接到时间嵌入（按颜色划分的 10 个斑点“类”）来添加文本调节。使用 0、5 和 9 级进行采样并验证颜色匹配。
3. **（困难）** 计算在相同数据上训练相同步数的相同大小网络的整流流和 DDPM 版本生成的样本之间的 Fréchet 距离（FID 代理）。收敛速度更快的报告。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
|迪特 | “扩散变压器”|替代U-Net作为扩散降噪器的Transformer；对修补后的潜在变量进行操作
|阿达LN | “自适应层规范”|通过在 LayerNorm 之后应用学习的比例、移位、门进行时间步长/文本调节；每个现代 DiT 的标准 |
| MMDiT | “多模态 DiT (SD3)” |共享联合自注意力的文本和图像标记的单独权重流 |
|单流/双流 | “通量技巧” |前 N 个块双流（每种模态单独的权重），后面块单流（concat + 共享权重）以提高效率 |
|整流流 | “直线噪声到数据”|数据和噪声之间的线性插值；网络预测速度；推理时所需的 ODE 步骤更少 |
|速度目标| “epsilon - x_0” |整流流中的回归目标；从干净数据到噪音点 |
| CFG指导| “无分类器指导” |混合条件和无条件预测；仍用于整流流模型|
|施奈尔/涡轮/LCM | “1-4步蒸馏”|从全质量模型中提取的小步变体；生产实时|

## 进一步阅读

- [使用 Transformers 的可扩展扩散模型 (Peebles & Xie, 2023)](https://arxiv.org/abs/2212.09748) — DiT 论文
- [缩放整流流变压器（Esser 等人，SD3 论文）](https://arxiv.org/abs/2403.03206) — MMDiT 和大规模整流流
- [FLUX.1模型卡和技术报告（Black Forest Labs）](https://huggingface.co/black-forest-labs/FLUX.1-dev) — 双流+单流详细信息
- [Z-Image：高效图像生成基础模型 (2025)](https://arxiv.org/html/2511.22699v1) — 6B 的单流 DiT
- [阐明扩散的设计空间 (Karras et al., 2022)](https://arxiv.org/abs/2206.00364) — 每个扩散设计权衡的参考
- [潜在一致性模型 (Luo et al., 2023)](https://arxiv.org/abs/2310.04378) — LCM-LoRA 如何为您提供 4 步推理
