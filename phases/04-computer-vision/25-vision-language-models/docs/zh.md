...
`apply_chat_template` 隐藏了 `<image>` 占位符标记化；模型在内部处理合并。

## 发货

本课产生：

- `outputs/prompt-vlm-selector.md` — 在给定精度、延迟、上下文长度和预算的情况下选择 Qwen3-VL / InternVL3.5 / LLaVA-Next / API。
- `outputs/skill-cmer-monitor.md` — 发出代码来检测生产 VLM 端点，包括跨模式错误率、每个端点仪表板和警报阈值。

## 练习

1. **（简单）** 通过任何打开的 VLM 在五个图像上运行三个提示（“这是什么？”、“计算对象”、“描述场景”）。手动将每个答案评分为正确/部分正确/幻觉。计算类似 CMER 的首次通过率。
2. **（中）** 使用 LoRA（排名 16）在带有标题的目标域的 500 张图像上微调 Qwen2.5-VL-3B 或 LLaVA-1.6-7B。比较零样本与微调 MMBench 式精度。
3. **（硬）** 将 VLM 的图像编码器替换为 DINOv3，而不是其默认的 SigLIP/CLIP。仅重新训练投影仪（冻结的 LLM + 冻结的 DINOv3）。衡量密集预测任务（计数、空间推理）是否有所改善。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
| ViT-MLP-法学硕士 | “VL​​M 模式”|视觉编码器+投影仪+语言模型；每 2026 年 VLM |
|投影仪| “桥”|将视觉标记映射到 LLM 嵌入空间的 2-4 层 MLP（或 Q-former）|
|深栈| 《Qwen3-VL功能技巧》|多层 ViT 功能堆叠而不仅仅是最后一层 |
|图像代币 | “<图像> 占位符”|文本流中的特殊标记被投影视觉嵌入取代 |
| CMER | “幻觉KPI”|跨模式错误率；当文本置信度较高但图像文本相似度较低时较高 |
|视觉代理| “点击的 VLM”|带工具调用的 VLM 操作 GUI（OSWorld、移动、Web）|
| Q-前| “固定计数令牌桥” | BLIP-2 式投影仪产生固定数量的视觉查询标记 |
|对齐/预训练/指令调优| “三个阶段” |标准 VLM 培训流程 |

## 进一步阅读

- [Qwen3-VL 技术报告 (arXiv 2511.21631)](https://arxiv.org/abs/2511.21631)
- [InternVL3.5 推进开源多模式模型 (arXiv 2508.18265)](https://arxiv.org/html/2508.18265v1)
...
- [MMMU：多学科多模式理解基准](https://mmmu-benchmark.github.io/)
- [制造业中的 VLM（明天机器人，2026 年 3 月）](https://www.roboticstomorrow.com/story/2026/03/when-machines-learn-to-see-like-experts-the-rise-of-vision-language-models-in-manufacturing/26335/)
