...
需要注意两件事：均值/标准差是 **数据集特定的** — 在 CIFAR-10 训练集上计算，而不是 ImageNet — 并且反射垫是社区默认的裁剪策略。在这里复制粘贴 ImageNet 统计数据会导致约 1% 的准确率泄漏，直到有人对模型进行分析之前，没有人发现这一点。
...
- `outputs/skill-classification-diagnostics.md` — 一项技能，在给定混淆矩阵和类名称列表的情况下，总结每个类的故障并提出最有影响力的修复方案。
...
3. **（困难）** 构建 CIFAR-100 管道（100 个类，相同的输入大小）并重现 ResNet-34 训练运行，使其达到已发布精度的 1% 以内。附加功能：扫描三个学习率和两个权重衰减，记录到本地 CSV，生成最终的混淆矩阵顶部混淆表。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
|洛吉特| “原始输出” |每个图像的 C 个数字的 pre-softmax 向量；交叉熵需要这些，而不是 softmaxed 值 |
|交叉熵 | “损失”|正确类别的负对数概率；将 log-softmax 和 NLL 结合在一个稳定的操作中 |
|数据加载器| “配料机” |通过洗牌、批处理和（可选）多工作人员加载来包装数据集；一半的训练错误被归咎于 |
|增强 | “随机变换” |训练时保留标签的任何像素级变换；教授 CNN 本身不具备的不变性 |
|混音/剪辑| “混合两个图像” |混合输入和标签，以便分类器学习平滑插值而不是硬边界 |
|标签平滑 | “软目标”|将 one-hot 替换为 (1-eps, eps/(C-1), ...)；改进校准并略微提高精度 |
| Top-k 准确率 | “前 5 名”|正确的类别属于 k 个最高概率的预测；用于具有真正模糊类的数据集|
|混淆矩阵 | “错误存在的地方”| C x C 表，其中条目 (i, j) 对预测为 j 的真实类别 i 的图像进行计数；对角线是正确的，非对角线告诉你要解决什么问题 |

## 进一步阅读

- [CS231n：训练神经网络](https://cs231n.github.io/neural-networks-3/) — 仍然是单页训练流程中最清晰的导览
- [图像分类技巧包 (He et al., 2019)](https://arxiv.org/abs/1812.01187) — 每个小技巧共同为 ImageNet 上的 ResNet 准确性增加 3-4%
- [mixup: Beyond Empirical Risk Minimization (Zhang et al., 2017)](https://arxiv.org/abs/1710.09412) — 原始 mixup 论文；三页理论加上令人信服的实验
- [为什么温度缩放很重要 (Guo et al., 2017)](https://arxiv.org/abs/1706.04599) — 这篇论文证明了现代网络的校准错误，并用一个标量参数修复了它
