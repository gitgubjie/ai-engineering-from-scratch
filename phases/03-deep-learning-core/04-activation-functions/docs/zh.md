...
变压器中的隐藏层：GELU。 CNN 中的隐藏层：ReLU。分类输出层：softmax。回归输出层：无（线性）。概率输出层：sigmoid。就是这样。从这些默认值开始。仅当有证据时才更改它们。

RNN 和 LSTM 使用 tanh 表示隐藏状态，使用 sigmoid 表示门，但如果您今天从头开始构建，您可能不会使用 RNN。如果 ReLU 网络中的神经元正在死亡，请切换到 GELU。除非有特定原因，否则不要使用 Leaky ReLU——GELU 解决了死神经元问题并提供了更好的梯度流。

## 发货

本课产生：
- `outputs/prompt-activation-selector.md` -- 一个可重复使用的提示，帮助您为任何架构选择正确的激活函数

## 练习

1. 实现参数化 ReLU (PReLU)，其中负斜率 alpha 是可学习参数。在圆数据集上对其进行训练，并与固定 Leaky ReLU 进行比较。

2. 使用 50 层（而不是 10 层）运行梯度消失实验。绘制 sigmoid、tanh、ReLU 和 GELU 在每层的幅度。每个激活的信号在哪一层有效地达到零？

3. 实现 ELU（指数线性单元）：如果 x > 0，则 elu(x) = x；如果 x <= 0，则 alpha * (e^x - 1)。将其死神经元率与同一网络上的 ReLU 进行比较。

4. 构建一个在训练期间运行的“梯度健康监视器”：在每个时期，计算每层的平均梯度幅度。当任何图层的梯度低于 0.001 或超过 100 时打印警告。

5. 修改训练比较以使用第 01 课中的 XOR 数据集而不是圆圈。哪个激活在 XOR 上收敛得最快？为什么这与循环结果不同？

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
|激活函数 | “非线性部分” |应用于每个神经元输出的打破线性的函数，使网络能够学习非线性映射 |
|梯度消失 | “梯度在深度网络中消失”|当激活的导数小于 1 时，各层的梯度呈指数收缩，使得早期层无法训练 |
|爆炸梯度| “渐变爆炸”|当有效乘数超过1时，梯度逐层呈指数增长，导致训练不稳定 |
|死亡神经元| “停止学习的神经元”|输入永久为负的 ReLU 神经元，产生零输出和零梯度 |
|乙状结肠 | “将值压缩为 0-1” |逻辑函数 1/(1+e^-x)，历史上很重要，但会导致深度网络中的梯度消失 |
| ReLU | “将底片剪辑为零”| max(0, x)——通过保留梯度幅度使深度学习变得实用的激活 |
|格鲁| “变压器激活” |高斯误差线性单元，一种平滑激活，通过输入为正的概率对输入进行加权 |
| Swish/SiLU| “自门控 ReLU”| x * sigmoid(x)，通过自动搜索发现，用于 EfficientNet |
| Softmax| “将分数转化为概率”|将 logits 向量标准化为概率分布，其中所有值都在 (0,1) 中且总和为 1 |
|泄漏 ReLU | “不会消亡的 ReLU”| max(alpha*x, x)，其中 alpha 较小 (0.01)，通过允许较小的负梯度来防止神经元死亡 |
|饱和度| “Sigmoid 的平坦部分”|激活导数接近零、阻止梯度流的区域 |
|洛吉特 | “softmax 之前的原始分数” |应用 softmax 或 sigmoid 之前最后一层的非标准化输出 |

## 进一步阅读

- Nair 和 Hinton，“整流线性单元改进受限玻尔兹曼机”(2010)——这篇论文介绍了 ReLU 并支持深度网络的训练
- Hendrycks 和 Gimpel，“高斯误差线性单位 (GELU)”(2016)——引入了成为 Transformer 默认值的激活函数
- Ramachandran 等人，“Searching for Activation Functions”（2017）——使用自动搜索发现 Swish，表明激活设计可以自动化
- Glorot 和 Bengio，“理解训练深度前馈神经网络的难度”（2010 年）——这篇论文诊断了梯度消失/爆炸并提出了 Xavier 初始化
- Goodfellow、Bengio、Courville，《深度学习》第 6.3 章（https://www.deeplearningbook.org/）——隐藏单元和激活函数的严格处理
