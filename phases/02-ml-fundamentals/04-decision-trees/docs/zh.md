...
在实践中，梯度提升树（XGBoost、LightGBM、CatBoost）通常比随机森林更强，因为它们按顺序构建树，每棵树都会纠正前一棵树的错误。但随机森林更难配置错误，并且几乎不需要超参数调整。

## 发货

本课程生成“outputs/prompt-tree-interpreter.md”——为业务利益相关者解释决策树拆分的提示。向其提供训练有素的树结构（深度、特征、分割阈值、准确性），它将模型转换为简单语言规则，对特征重要性进行排名，标记过度拟合或泄漏，并建议后续步骤。任何时候您需要向不读代码的人解释基于树的模型时都可以使用它。

## 练习

1. 在具有 3 个类别的 2D 数据集上训练单个决策树。手动追踪分割并绘制矩形决策边界。比较 max_深度 = 2 与 max_深度 = 10 处的边界。

2. 对回归树实施方差缩减分裂。生成 200 个点的 y = sin(x) + 噪声并拟合您的回归树。根据真实曲线绘制树的分段常数预测。

3. 构建一个有 1、5、10、50 和 200 棵树的随机森林。绘制训练精度和测试精度与树数量的关系图。观察到测试精度稳定但没有下降（森林可以抵抗过度拟合）。

4. 比较基尼杂质与熵作为 5 个不同数据集的分割标准。测量精度和树深度。在大多数情况下，它们产生几乎相同的结果。解释一下为什么。

5. 实施排列重要性。将其与数据集上的 MDI 重要性进行比较，其中一个特征是随机噪声但具有高基数。 MDI 会将噪声特征排在较高位置。排列重要性不会。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
|决策树| “预测流程图”|通过学习 if/else 分割序列将特征空间划分为矩形区域的模型 |
|基尼杂质| “节点的混合程度如何”|在节点处对随机样本进行错误分类的概率。 0 = 纯净，0.5 = 二进制的最大杂质 |
|熵| “节点中的无序”|节点的信息内容。 0 = 纯，1.0 = 二进制的最大不确定性。来自信息论|
|信息增益| “分裂有多好” |分裂后杂质减少。选择分割的贪心准则 |
|预剪枝| “早点把树停掉” |通过设置最大深度、最小样本或最小增益阈值提前停止树木生长 |
|后剪枝| “之后修剪树”|生长完整的树，然后删除不会提高验证性能的子树 |
|装袋| “训练随机子集”|引导聚合。在不同的随机样本上训练每个模型并进行替换 |
|随机森林| “一堆树”|决策树集合，每个决策树都在引导样本上进行训练，每次分割时都具有随机特征子集 |
|特征重要性 (MDI) | “哪些功能很重要”|每个特征贡献的总杂质减少量，对所有树和节点求和 |
|排列重要性| “洗牌并检查”|当特征值被随机打乱时，准确性会下降。对于噪声特性比 MDI 更可靠 |
|方差减少 | “信息增益的回归版本”|信息增益的回归树模拟。选择最大程度减少目标方差的分割 |
|引导样本 | “具有重复的随机样本”|从原始数据集中替换后抽取的随机样本。大小相同，但有重复 |

## 进一步阅读

- [Breiman: Random Forests (2001)](https://link.springer.com/article/10.1023/A:1010933404324) - 原始随机森林论文
- [Grinsztajn 等人：为什么基于树的模型在表格数据上仍然优于深度学习？ (2022)](https://arxiv.org/abs/2207.08815) - 表格任务上树与神经网络的严格比较
- [scikit-learn 决策树文档](https://scikit-learn.org/stable/modules/tree.html) - 可视化工具实用指南
