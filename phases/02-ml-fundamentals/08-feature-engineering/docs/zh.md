...
从头开始的版本准确地显示了每个转换内部发生的情况。库版本添加了边缘情况处理、稀疏矩阵支持和管道组合，但数学原理是相同的。

## 发货

本课产生：
- `outputs/prompt-feature-engineer.md` - 从原始数据系统地设计特征的提示

## 练习

1. 在数值变换中添加稳健的缩放（使用中位数和四分位数范围而不是平均值和标准差）。将其与具有极端异常值的数据的标准缩放进行比较。
2. 实施留一法目标编码：对于每一行，计算目标均值，排除该行自己的目标值。展示与朴素目标编码相比，这如何减少过度拟合。
3. 构建一个结合了方差阈值、相关性过滤和互信息排序的自动化特征选择管道。将其应用于住房数据集，并将模型性能（使用简单的线性回归）与所有特征与选定特征进行比较。

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
|特征工程 | “制作新专栏” |将原始数据转换为向模型公开模式的表示形式 |
|标准化| “让它变得正常”|减去平均值并除以标准差，因此该特征的平均值 = 0 且 std = 1 |
| One-hot 编码 | “制作虚拟变量” |为每个类别创建一个二进制列，其中每行恰好有一列为 1 |
|目标编码| “使用答案进行编码”|将每个类别替换为该类别的平均目标值，并进行平滑以防止过度拟合 |
| TF-IDF | “花哨的字数”|词频乘以逆文档频率：根据词在语料库中的独特程度进行加权 |
|插补| “填空”|用估计值（平均值、中位数、众数或模型预测）替换缺失值 |
|特征选择| “扔掉坏专栏”|删除增加噪音或冗余的特征，仅保留那些与目标相关的信号 |
|互信息 | “一件事告诉你多少关于另一件事的信息” |通过观察变量 X 获得的变量 Y 不确定性减少的度量 |
|数据泄露| “不小心作弊”|在训练期间使用在预测时无法获得的信息，给出错误乐观的结果 |

## 进一步阅读

- [特征工程和选择 (Max Kuhn & Kjell Johnson)](http://www.feat.engineering/) - 免费在线书籍，涵盖特征工程的全部内容
