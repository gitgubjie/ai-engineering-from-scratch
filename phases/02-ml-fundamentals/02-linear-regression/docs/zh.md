...
您的从头开始实现和 scikit-learn 会产生相同的结果。区别：scikit-learn 处理边缘情况、数值稳定性和性能优化。使用该库进行生产。使用从头开始的版本来了解正在发生的情况。
...
1. 实现批量梯度下降、随机梯度下降（SGD）和小批量梯度下降。比较同一数据集上的收敛速度。哪个收敛最快？哪个成本曲线最平滑？
2. 从三次函数生成数据 (y = ax^3 + bx^2 + cx + d + 噪声)。拟合 1、3 和 10 次多项式。比较训练 R^2 和测试 R^2。过度拟合到什么程度才会变得明显？
3. 实现Lasso回归（L1正则化：penalty = alpha * sum(|w_i|)）。对多功能住房数据进行训练。比较哪些权重归零与 Ridge。为什么 L1 产生稀疏解而 L2 却不能？

## 关键术语

|术语 |人们怎么说 |它实际上意味着什么 |
|------|----------------|----------------------|
|线性回归| “通过数据画一条线” |找到权重 w 和偏差 b，使 wx+b 与实际 y 值之间的平方差最小化 |
|成本函数| “这个模型有多糟糕”|将模型参数映射到测量预测误差的单个数字的函数，该函数优化可最小化 |
|均方误差| “平方误差的平均值” | (1/n) * (预测 - 实际)^2 之和，不成比例地惩罚大错误 |
|梯度下降| 《走下坡路》|使用偏导数朝降低成本函数的方向迭代调整参数 |
|学习率| “步长”|控制每个梯度下降步骤参数变化量的标量 |
|正规方程| 「直接解决」|闭合形式解 w = (X^T X)^-1 X^T y 无需迭代即可给出最佳权重 |
| R 平方 | “多么合身”|模型解释的 y 方差分数，范围从负无穷大到 1.0 |
|特征缩放| “使功能具有可比性” |将特征转换为相似的范围（例如，零均值、单位方差），以便梯度下降收敛得更快 |
|正则化| “惩罚复杂性”|在成本函数中添加一项来缩小权重，防止过度拟合 |
|岭回归| “L2 正则化”|线性回归，并在 MSE 中添加 lambda * sum(w_i^2) 惩罚 |
|多项式回归 | “用线性数学拟合曲线” |多项式特征 (x, x^2, x^3, ...) 的线性回归，权重仍然是线性的 |
|过度拟合 | “记忆训练数据” |使用如此复杂的模型，以至于它适合训练数据中的噪声，但无法适应新数据 |

## 进一步阅读

- [统计学习简介 (ISLR)](https://www.statlearning.com/) -- 免费 PDF，第 3 章和第 6 章涵盖线性回归和正则化以及实际的 R 示例
- [统计学习要素 (ESL)](https://hastie.su.domains/ElemStatLearn/) -- 免费 PDF，ISLR 的数学伴侣，对岭和套索进行更深入的处理
- [Stanford CS229 Linear Regression 讲座笔记](https://cs229.stanford.edu/main_notes.pdf) -- Andrew Ng 从第一原理推导出正规方程和梯度下降的笔记
- [scikit-learn LinearRegression 文档](https://scikit-learn.org/stable/modules/linear_model.html) -- LinearRegression、Ridge、Lasso 和 ElasticNet 的实用参考以及代码示例
